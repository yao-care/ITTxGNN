...
def load_fda_drugs(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    Args:
        filepath: JSON 檔案路徑，預設為 data/raw/it_fda_drugs.json

    Returns:
        包含所有藥品的 DataFrame

    Raises:
        FileNotFoundError: 找不到 AIFA 資料檔案時，提供下載指引
    """
    if filepath is None:
        # loader.py -> data -> ittxgnn -> src -> ITTxGNN
        filepath = Path(__file__).parent.parent.parent.parent / "data" / "raw" / "it_fda_drugs.json"

    if not filepath.exists():
        raise FileNotFoundError(
            f"找不到 AIFA 藥品資料: {filepath}\n"
            f"請先執行以下步驟：\n"
