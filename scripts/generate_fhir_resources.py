...
BASE_URL = "https://ittxgnn.yao.care"
...
def generate_capability_statement() -> dict:
    ...
    return {
        ...
        "id": "ittxgnn-fhir-server",
        "url": f"{BASE_URL}/fhir/metadata",
        "version": "1.0.0",
        "name": "ITTxGNNFHIRServer",
        "title": "ITTxGNN FHIR Server - Italy Drug Repurposing",
        "description": "FHIR R4 server for Italy TxGNN drug repurposing predictions",
        "status": "active",
        "date": datetime.now().strftime("%Y-%m-%d"),
        "publisher": "TxGNN Project",
        "kind": "instance",
        "fhirVersion": "4.0.1",
        "format": ["json"],
        "rest": [{
...
def generate_clinical_use_definition(
    ...
    indication_slug = indication.lower().replace(" ", "-").replace("/", "-")
